...
namespace NdGameSdk::game::player {
	...
	PlayerMenu::PlayerMenu() : ISdkComponent("DMENU::PlayerMenu") {}
	...
	void PlayerMenu::Awake() {
		m_DMENU = ISdkComponent::GetSharedComponents()->GetComponent<gamelib::debug::NdDevMenu>();
		m_WeaponProcess = ISdkComponent::GetNdGameComponents()->GetComponent<weapon::Weapon>();
	}
	...
	void PlayerMenu::Initialize() {
		...
		std::call_once(Initialized, [this] {
			...
			auto MissingDependencies = ISdkComponent::CheckSdkComponents
				<gamelib::debug::NdDevMenu, weapon::Weapon>({ m_DMENU.get(), m_WeaponProcess.get() });

			if (MissingDependencies.has_value()) {
				throw SdkComponentEx
				{ std::format("Couldn't initialize without {:s} dependencies components.", MissingDependencies.value()),
			...
			if (m_DMENU->IsGameDebugMenu()) {
				...
				GiveWeaponArtFunctAddr[0x1] = reinterpret_cast<uintptr_t>(m_WeaponProcess.get());
